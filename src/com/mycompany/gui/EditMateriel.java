 ...
 * @author bhk
 */
public class EditMateriel extends Form{
    
   String filePath;
   Form detailsForm;
   String Imagecode;
    public EditMateriel(Form previous,Materiels v) {
        setTitle("Modifier Materiels");
        setLayout(BoxLayout.y());
        TextComponent tftype= new TextComponent().label("Type");
        tftype.text(v.getType());
        ...
        tfprix.text(String.valueOf(v.getPrix_jour()));
        TextComponent tfddd= new TextComponent().label("Date Debut");
        Picker datePicker = new Picker();
        datePicker.setType(Display.PICKER_TYPE_DATE);
        datePicker.setDate(v.getDateDebut());
        ...
        datePickerr.setDate(v.getDateFin());
        
        Label l2 = new Label("Diponibiliter :");
        ComboBox<String> combo = new ComboBox<>();
        combo.addItem("Disponible");
        combo.addItem("Non Disponible");
        if(v.isDisponibilite()==true)
        {
        combo.setSelectedItem("Disponible");
        }
        else
        {
        combo.setSelectedItem("Non Disponible");
        }
            ...
            ByteArrayOutputStream out = new ByteArrayOutputStream();
                    try {
                        ...
                        Imagecode = Base64.encode(ba);
                           ...
                           Materiels t = new Materiels(v.getId(),tftype.getText(), tfdescription.getText(),Float.parseFloat(tfprix.getText()),false, filePath, datePicker.getDate(),datePicker.getDate());
                        boolean disp =false;                           
                        ...
                        {
                         t.setDisponibilite(false);
                        }
                           if( ServiceMateriels.getInstance().editMateriels(t)){
                               {
        ...
        getToolbar().addMaterialCommandToLeftBar("", FontImage.MATERIAL_ARROW_BACK, e-> new ListMateriels().show());
